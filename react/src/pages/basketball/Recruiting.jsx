...
export default function BasketballRecruiting() {
    const { user, loading } = useAuth();
    const [games, setGames] = useState([]);
    const isAdmin = isAdminUser(user);

    useMeta("Basketball Recruiting Profile | Veyro", "Build a basketball recruiting profile with match ratings and stats to share with college coaches and scouts.");
    useEffect(() => {
    ...
    return (
        <main className={`page ${styles.appPage}`}>
            <section className={styles.hero}>
                <div><div className={styles.eyebrow}>Basketball</div><h1 className={styles.h1}>Basketball Recruiting</h1><p>Build a recruiting profile from saved data.</p>
