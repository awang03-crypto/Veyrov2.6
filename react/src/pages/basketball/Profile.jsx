...
export default function BasketballProfile() {
    const { user, loading } = useAuth();
    const [games, setGames] = useState([]);
    const isAdmin = isAdminUser(user);

    useMeta("Basketball Player Profile | Veyro", "Your basketball player profile with average rating, best position, and full match history.");
    useEffect(() => {
    ...
    return (
        <main className={`page ${styles.appPage}`}>
            <section className={styles.hero}>
                <div><div className={styles.eyebrow}>Basketball</div><h1 className={styles.h1}>Basketball Profile</h1><p>Build a player profile from saved ratings.</p>
